...
room.onCommand_stats_help = () => {
    room.sendChat("distr: Distribution of ball on pitch, divided into three areas. poss: Possession by teams. poss players: Possession by players.");
};
...
room.outputPassesPerPlayer = () => {
    ...
    for (let player in passes) {
        ...
        output = output + " " + room.getPlayer(player).name + ": " + passes[player].succ + "/" + passes[player].overall + "=" + percentage + "%,";
    }
    output = output.slice(0, -1);
    return output;
}
